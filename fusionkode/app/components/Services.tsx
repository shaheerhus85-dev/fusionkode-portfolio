...
function ServiceCard({ service, index }: { service: typeof services[0], index: number }) {
    ...
    return (
        <div
            ...
            className="service-card glass-panel group relative p-10 md:p-12 overflow-hidden flex flex-col gap-10 cursor-pointer will-change-transform h-full"
        >
            {/* Mouse Glow */}
            <div
                ref={glowRef}
                ...
                style={{
                    background: 'radial-gradient(400px circle at 0px 0px, rgba(255,255,255,0.08), transparent 80%)',
            ...
            <div className="relative z-20 flex flex-col gap-10">
                ...
                <div className="flex flex-col gap-4">
                    <h4 className="font-display text-2xl md:text-3xl font-bold uppercase tracking-tight text-white leading-tight">
                        {service.title}
                    </h4>
                    <p className="font-sans text-sm md:text-base text-white/40 leading-relaxed group-hover:text-white/60 transition-colors">
                        {service.desc}
                    </p>
                </div>
            </div>

            <div className="relative z-20 mt-4 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-all duration-700 translate-y-4 group-hover:translate-y-0 text-white">
                <span className="btn-glow-outline px-4 py-2 font-mono text-[10px] uppercase tracking-widest text-white/40 group-hover:text-white transition-colors border border-white/10 rounded-full">
                    Deploy Specs →
                </span>
            </div>
        </div>
...
export default function Services() {
    ...
    return (
        <section
            ref={containerRef}
            id="services"
            className="relative w-full py-24 md:py-48 bg-[#000000] px-6 md:px-12 flex flex-col items-center"
        >
            <div className="w-full max-w-[1320px]">
                <div className="mb-24 flex flex-col gap-8 text-center md:text-left">
                    <div className="flex flex-col gap-4">
                        ...
                        <h3 className="font-display font-bold uppercase tracking-tighter text-white leading-[0.9]" style={{ fontSize: 'clamp(32px, 6vw, 100px)' }}>
                            ENGINEERING <br />
                            THE FUTURE.
                        </h3>
                    </div>
